...
mkdir -p $WEB_ROOT
mkdir -p $WEB_ROOT/default/www
mkdir -p $WEB_ROOT/example.com/www
...
box server start name=default port=8080 host=127.0.1.1 cfengine=lucee serverConfigFile=${WEB_ROOT}/default/server.json directory=${WEB_ROOT}/default/www rewritesEnable=false openbrowser=false saveSettings=true --force;